...
function ProfilePage() {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:8000/api/users/appointments/', {
          headers: {
  ...
  const flagAppointment = async (appointmentId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`http://localhost:8000/api/appointments/${appointmentId}/flag/`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      setMessage('Appointment flagged successfully');
    } catch (error) {
        ...
        <TableBody>
          {appointments.map((appointment) => (
            <TableRow key={appointment.id}>
              ...
              <TableCell>
                <Button onClick={() => flagAppointment(appointment.id)} disabled={appointment.status === 'flagged'}>
                  Flag
                </Button>
              </TableCell>
            </TableRow>
